...
import QuanlityList from "./List/quanlityList";
import Fillter from "./Fillter/quanlityFillter";
...
const quanlity = memo(({}) => {
  ...
  const exportExcel = useCallback(async () => {
    ...
    if (result.hasErrors) {
      ...
      downloadFile(result.data, "Báo cáo quản lý chất lượng");
      Ui.showSuccess({ message: "Xuất Excel thành công" });
    }
  }, [params]);
  const bowserLoad = useCallback(async () => {
    let organizationIds = _.isArray(params.organizationIds)
      ? params.organizationIds.map((item, key) => {
          return item.key;
        })
    ...
    if (result.hasErrors) {
      ...
      setGird((preState) => {
        ...
        nextState = nextState.set("data", result.data.data);
  ...
  useEffect(() => {
    clearTimeout(time);
    time = setTimeout(bowserLoad, 800);
  }, [bowserLoad]);
  return (
